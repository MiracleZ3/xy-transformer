...
v2 单种子结果有两个被反馈指出的问题：① 验证曲线在 epoch 10 后明显震荡；② 基线方法没披露训练
过程给人不可信感。v3 做三项整改：

| 整改 | 解决的问题 |
|---|---|
| **多 seed 重复**（默认 5 个，报告 mean ± std） | 单种子的随机性遮盖了真实信号强弱 |
| **early-stop + 调小 lr (1e-3→3e-4) + 加大 dropout/weight_decay** | val 震荡；在小数据集 (1050 样本) 上稳定收敛 |
| **LightGBM 用相同 seed 集合** | 三方法口径一致，对比公平；不给人"凭空捏造"质疑空间 |
| **训练曲线画 mean ± std 区间**（不是单条曲线） | 视觉上看得出波动范围，不再误导为"完美收敛" |
| **WAPE 图改误差棒 + 新增相对提升% 子图** | 突出三方法相对差异，避免"看不出区别" |

**重要执行范式**：本工作分两种跑法——
- **CPU 烟雾测试**（脚本正确性验证）：`simulate_xy_real_schema.py --small + train --epochs 2 --seeds 2`，
  约 3 分钟跑完，**只用于验证代码不崩，数字不作为结论**
- **A800×8 全量训练**（正式结果）：见 [`run_on_a800.md`](../run_on_a800.md)，约 20–40 分钟，
  **数字才能真正填入本节结论表**

> **当前 docs/assets/ 下的图是 CPU 烟雾产物，不可作为结论**——plot 脚本若检测到训练未收敛会在
> fig4 上自动叠加红色"训练未收敛"警告。请在 A800 上跑全量后再用 PNG 替换。
